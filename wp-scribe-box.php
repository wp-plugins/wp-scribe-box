	...
	?>
		<div class="wrap">
			...
			<table class="widefat">
				<thead>
					<tr>
						<th title="<?php _e('The name of the parameter', WPSB_LOCAL); ?>"><strong><?php _e('Parameter Name', WPSB_LOCAL); ?></strong></th>
						<th title="<?php _e('Is this parameter required?', WPSB_LOCAL); ?>"><strong><?php _e('Is Required?', WPSB_LOCAL); ?></strong></th>
						<th title="<?php _e('What data type this parameter accepts', WPSB_LOCAL); ?>"><strong><?php _e('Data Type', WPSB_LOCAL); ?></strong></th>
						<th title="<?php _e('What, if any, is the default if no value is specified', WPSB_LOCAL); ?>"><strong><?php _e('Default Value', WPSB_LOCAL); ?></strong></th>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						...
						<td><?php 
							if ($plugin_defaults_values[$i] === true) {
								...
								echo '<em>';
								_e('this value is blank by default', WPSB_LOCAL);
								echo '</em>';
			...
			<h3 id="support"><img src="<?php echo plugins_url(plugin_basename(dirname(__FILE__) . '/images/support.png')) ?>" title="" alt="" height="64" width="64" align="absmiddle" /> Support</h3>
				...
				<?php echo '<a href="http://wordpress.org/extend/plugins/' . WPSB_SLUG . '/">' . __('Documentation', WPSB_LOCAL) . '</a> | ';
					...
					_e('If you like this plugin, please ', WPSB_LOCAL);
					echo '<a href="http://wordpress.org/support/view/plugin-reviews/' . WPSB_SLUG . '/">';
					_e('rate it on WordPress.org', WPSB_LOCAL);
					echo '</a> ';
					_e('and click the ', WPSB_LOCAL);
					echo '<a href="http://wordpress.org/plugins/' . WPSB_SLUG .  '/#compatibility">';
					_e('Works', WPSB_LOCAL);
					echo '</a> ';
					_e('button. For support please visit the ', WPSB_LOCAL);
					echo '<a href="http://wordpress.org/support/plugin/' . WPSB_SLUG . '">';
					_e('forums', WPSB_LOCAL);
					echo '</a>.';
				?>
				<br /><br />
				...
				<br /><br />
				<small><?php _e('Disclaimer: This plugin is not affiliated with or endorsed by ShareASale or CopyBlogger Media.', WPSB_LOCAL); ?></small>
				</div>
		</div>
		<?php 
	}
		...
		if ($enabled) {
			...
			if ($affiliate_url === WPSB_DEFAULT_URL) { // no url passed to function, try settings page
				...
				if (($affiliate_url === WPSB_DEFAULT_URL) || ($affiliate_url === false)) { // no url on settings page either
					$enabled = false;
					$output = '<!-- ' . WPSB_PLUGIN_NAME . ': ' . __('plugin is disabled. Either you did not pass a necessary setting to the plugin, or did not configure a default. Check Settings page.', WPSB_LOCAL) . ' -->';
				}
			}
